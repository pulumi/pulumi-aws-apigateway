 ...
 */
export function createAssociatedAPIKeys(
  name: string,
  args: APIKeyArgs,
...
function getUsagePlan(
  ...
  if (args.usagePlan && pulumi.CustomResource.isInstance(args.usagePlan)) {
    if (args.apis) {
      throw new Error(
        "cannot define both [args.apis] and an existing usagePlan [args.usagePlan]"
      );
    }
    return args.usagePlan;
  } else {
    let usagePlanArgs: aws.apigateway.UsagePlanArgs | undefined =
      args.usagePlan;
    ...
    if (args.apis) {
      ...
      if (args.usagePlan) {
        if (args.usagePlan.apiStages) {
          throw new Error(
            "cannot define both [args.apis] and [args.usagePlan.apiStages]"
          );
        }
    ...
    return new aws.apigateway.UsagePlan(
      name,
      usagePlanArgs,
  ...
  if (args.apiKeys) {
    for (let i = 0; i < args.apiKeys.length; i++) {
      ...
      const apikey = pulumi.CustomResource.isInstance(currKey)
        ? currKey
        : new aws.apigateway.ApiKey(childName, currKey, {
            aliases: [{ parent: pulumi.rootStackResource }],
            parent: usagePlan,
          });
