...
/** @internal */
export function isLambdaAuthorizer(
  authorizer: LambdaAuthorizer | CognitoAuthorizer
): authorizer is LambdaAuthorizer {
  return (<LambdaAuthorizer>authorizer).handler !== undefined;
}

/** @internal */
export function isLambdaAuthorizerInfo(
  info:
    | LambdaAuthorizerInfo
    | aws.lambda.EventHandler<AuthorizerEvent, AuthorizerResponse>
): info is LambdaAuthorizerInfo {
  return (<LambdaAuthorizerInfo>info).uri !== undefined;
}

/** @internal */
export function getIdentitySource(
  identitySources: string[] | undefined
): string {
  if (identitySources) {
...
export function createRoleWithAuthorizerInvocationPolicy(
  authorizerName: string,
  authorizerLambda: aws.lambda.Function,
  opts: pulumi.CustomResourceOptions
): aws.iam.Role {
  const policy = aws.iam.assumeRolePolicyForPrincipal({
  ...
  const role = new aws.iam.Role(
    authorizerName + "-authorizer-role",
    {
  ...
  const invocationPolicy = new aws.iam.RolePolicy(
    authorizerName + "-invocation-policy",
    {
 ...
 */
export function authorizerResponse(
  principalId: string,
  effect: Effect,
  resource: string,
 ...
 */
export function getTokenLambdaAuthorizer(
  args: TokenAuthorizerArgs
): LambdaAuthorizer {
  return {
 ...
 * @param args - configuration information for the token Lambda.
 */
export function getRequestLambdaAuthorizer(
  args: RequestAuthorizerArgs
): LambdaAuthorizer {
  ...
  if (numQueryParams === 0 && numHeaders === 0) {
    throw new Error(
      "[args.queryParameters] and [args.headers] were both empty. At least one query parameter or header must be specified"
    );
  } else {
