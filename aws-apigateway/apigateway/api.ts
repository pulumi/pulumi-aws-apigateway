...
export interface IntegrationRouteTargetProvider {
  ...
  ): pulumi.Input<IntegrationTarget>;
}

function isIntegrationRouteTargetProvider(
  obj: any
): obj is IntegrationRouteTargetProvider {
...
export function createAPI(
  ...
  if (args.swaggerString) {
    ...
    const result = createSwaggerSpec(
      ...
      name,
      args.routes || [],
      pulumi.output(args.additionalRoutes || []),
      args.gatewayResponses,
    ...
    throw new pulumi.ResourceError(
      "API must specify either [swaggerString] or as least one of the [route] options.",
      optsParent
    );
  }
  ...
  const restAPI = new aws.apigateway.RestApi(
    name,
    {
      ...
      binaryMediaTypes: binaryMediaTypesList,
      body: swaggerString,
  ...
  if (restApiArgs.policy) {
    apiPolicy = new aws.apigateway.RestApiPolicy(
      name,
      {
      ...
      permissions.push(
        new aws.lambda.Permission(
          name + "-" + sha1hash(methodAndPath),
          {
            ...
            sourceArn: pulumi.interpolate`${restAPI.executionArn}/${stageName}*/${methodAndPath}`,
          },
          { parent }
        )
      );
...
function createSwaggerSpec(
  ...
  const swagger: SwaggerSpec = {
    ...
    paths: {},
    "x-amazon-apigateway-binary-media-types": binaryMediaTypesList,
    ...
    "x-amazon-apigateway-gateway-responses":
      generateGatewayResponses(gatewayResponses),
  ...
  for (const route of routes) {
    ...
    if (isEventHandler(route)) {
      addEventHandlerRouteToSwaggerSpec(
        parent,
        name,
        swagger,
        swaggerLambdas,
  ...
  function addIntegrationOrRawDataRouteToSwaggerSpec(
...
function generateGatewayResponses(
  responses: Record<string, SwaggerGatewayResponse> | undefined
): Record<string, SwaggerGatewayResponse> {
  responses = responses || {};
...
function checkRoute<TRoute>(
  parent: pulumi.Resource,
  route: TRoute,
  propName: keyof TRoute
) {
  if (route[propName] === undefined) {
    throw new pulumi.ResourceError(
      `Route missing required [${String(propName)}] property`,
      parent
    );
  }
}
...
function addEventHandlerRouteToSwaggerSpec(
  ...
  const lambda = aws.lambda.createFunctionFromEventHandler(
    name + sha1hash(method + ":" + route.path),
    route.eventHandler,
    { parent }
  );
...
function addBasePathOptionsToSwagger(
  ...
  if (route.requiredParameters) {
    addRequiredParametersToSwaggerOperation(
      swaggerOperation,
      route.requiredParameters
    );
  }
  if (route.requestValidator) {
    swaggerOperation["x-amazon-apigateway-request-validator"] =
...
function addAPIkeyToSecurityDefinitions(swagger: SwaggerSpec) {
  ...
  if (
    swagger.securityDefinitions["api_key"] &&
    swagger.securityDefinitions["api_key"] !== apiKeySecurityDefinition
  ) {
    throw new Error(
      "Defined a non-apikey security definition with the name api_key"
    );
  }
  swagger.securityDefinitions["api_key"] = apiKeySecurityDefinition;
}
  ...
  for (const auth of authorizers) {
    ...
    if (!swagger.securityDefinitions[auth.authorizerName]) {
      swagger.securityDefinitions[authName] = {
        ...
        in: lambdaAuthorizer.isLambdaAuthorizer(auth)
          ? auth.parameterLocation
          : "header",
        "x-amazon-apigateway-authtype": lambdaAuthorizer.isLambdaAuthorizer(
          auth
        )
          ? auth.authType
          : "cognito_user_pools",
        "x-amazon-apigateway-authorizer": lambdaAuthorizer.isLambdaAuthorizer(
          auth
        )
    ...
    const methods =
      lambdaAuthorizer.isLambdaAuthorizer(auth) || !auth.methodsToAuthorize
        ? []
...
function getCognitoPoolARNs(
  pools: (pulumi.Input<string> | aws.cognito.UserPool)[]
): pulumi.Input<string>[] {
...
function getLambdaAuthorizer(
  parent: pulumi.Resource,
  authorizerName: string,
  authorizer: lambdaAuthorizer.LambdaAuthorizer
): SwaggerLambdaAuthorizer {
  if (lambdaAuthorizer.isLambdaAuthorizerInfo(authorizer.handler)) {
    const identitySource = lambdaAuthorizer.getIdentitySource(
  ...
  const authorizerLambda = aws.lambda.createFunctionFromEventHandler(
    authorizerName,
    authorizer.handler,
    {
      parent,
  ...
  const role = lambdaAuthorizer.createRoleWithAuthorizerInvocationPolicy(
    authorizerName,
    authorizerLambda,
    {
      parent,
...
function addAuthorizersToSwaggerOperation(
  swaggerOperation: SwaggerOperation,
  authRecords: Record<string, string[]>[]
) {
  swaggerOperation.security = swaggerOperation.security || [];
...
function addRequiredParametersToSwaggerOperation(
  swaggerOperation: SwaggerOperation,
  requiredParameters: reqvalidation.Parameter[]
) {
...
function addStaticRouteToSwaggerSpec(
  ...
  function processDirectory(directory: StaticRoute) {
    const directoryServerPath = route.path.endsWith("/")
      ? route.path
      : route.path + "/";
    ...
    function walk(dir: string) {
      ...
      for (const childName of children) {
        ...
        if (stats.isDirectory()) {
          ...
          const childRelativePath = childPath
            .substr(startDir.length)
            .split(fspath.sep)
            .join(fspath.posix.sep);
          ...
          if (childPath === indexPath) {
            ...
            const swaggerOperation = createSwaggerOperationForObjectKey(
              childUrn,
              role
            );
...
function addIntegrationRouteToSwaggerSpec(
  ...
  const target = isIntegrationRouteTargetProvider(route.target)
    ? pulumi.output(route.target.target(name + sha1hash(route.path), parent))
  ...
  const swaggerOpWithoutProxyPathParam = createSwaggerOperationForProxy(
    target,
    /*useProxyPathParameter:*/ false
  );
  addBasePathOptionsToSwagger(
    parent,
    swagger,
  ...
  const swaggerOpWithProxyPathParam = createSwaggerOperationForProxy(
    target,
    /*useProxyPathParameter:*/ true
  );
  addBasePathOptionsToSwagger(
    parent,
    swagger,
  ...
  function createSwaggerOperationForProxy(
    ...
    const passthroughBehavior = ifUndefined(
      target.passthroughBehavior,
      "when_no_match" as const
    );
